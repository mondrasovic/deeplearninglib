...
class Linear(Layer):
    ...
    def backward(self, grad: Tensor) -> Tensor:
        ...
        dy/da = f'(x) * b, which is dy/da = df/dx * dx/da
        and dy/db = f'(x) * a, which is dy/db = df/dx * dx/db
        and dy/dc = f'(x), which is dx dy/dc = df/dx * dx/dc
...
class Activation(Layer):
    """
    An activation just applies a function elementwise to its inputs.
    """
